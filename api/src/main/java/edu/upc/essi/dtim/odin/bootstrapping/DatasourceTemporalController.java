...
public class DatasourceTemporalController {
    ...
    public ResponseEntity<DataSource> createDataSource(Authentication authentication, @PathVariable("id") String id, @RequestPart DataSource dataSource, @RequestPart MultipartFile file) {
        LOGGER.debug("inside of createDataSource method");
        Project p = validateAccess(id, authentication);
        // just for security
        dataSource.setProjectID(id);
        try {
            DataSource _dataSource = dataSourceService.create(dataSource, file);
...
//            System.out.println( graph.temporal().getGraph(dataSource.getIri()).size());
    ...
    @GetMapping
    public ResponseEntity<List<DataSource>> getAllTemporalDataSources(Authentication authentication, @PathVariable("id") String id) {
        validateAccess(id, authentication);

        try {
            List<DataSource> dataSources = dataSourceRepo.findAllTemporal(id);
